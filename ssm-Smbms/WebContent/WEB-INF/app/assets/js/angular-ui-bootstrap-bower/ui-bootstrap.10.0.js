   ...
   *   });
    ...
    angular.module('ui.bootstrap.progressbar', ['ui.bootstrap.transition'])
        ...
        .directive('starRating', function(){
            return {
                restrict: 'E',
                templateUrl: 'template/starRating/starRating.html',
                link: function(scope){
                    scope.click = function(starRating) {
                        if(scope.clickRating!==false){
                    ...
                    scope.$watch('starRating', function(oldVal, newVal) {
                        if (newVal) {
                            ...
                            for(var i = 0; i < scope.maxStarRating; i++){
                                if(i >= starRating){
                                    scope.stars.push({empty:'off', index:i+1});
                                }else{
                                    if(parseInt(starRating)===starRating){
                                        scope.stars.push({empty:'on', index:i+1});
                                        console.log(starRating);
                                    }else{
                                        var star = starRating.substr(starRating.indexOf(".")+1);
                                        if(parseInt(star)!==0 && i==parseInt(starRating)){
                                            scope.stars.push({empty:'half', index:i+1});
                                        }else{
                                            scope.stars.push({empty:'on', index:i+1});
                                        }
                                    }
                        ...
                        if(scope.starSize == null ||scope.starSize ==undefined ){
                            scope.starSize = 20;
                        }
                    });
                },
